...
% Ya Hussain (a.s.)
%===================
% The presented code simulates the MAS control with obstacles
%===================
% Preparing MATLAB
...
NOA = 8;         % Number of Agents 
Snum = 1000;     % number of samples per iteration
iteration = 100; % number of algorithm iteration
...
Obs_placer = [35,0,0]';
x_obs = [35,10/2 + 5 ,5]' + Obs_placer;
x_obs2 = [25,14/2 + 5,5]' + Obs_placer;
...
for ii = 1:E 
  ...
  % Flocking Optimal Position Computation
    ...
    Xmat_B(3,:) = zeros(1,NOA);       % Projections onto Barrier surface
    Xmat_BPro = Xmat_B;
  ...
  for j = 1:NOA
      ...
      % obstacle detection
       ...
       for i = 1:4
           ...
           OD_metric_range = norm([X_pos(1) - X_obs(1,i), X_pos(2) - X_obs(2,i), X_pos(3) - X_obs(3,i)]);
           % presence in sensor FOV
           OD_metric_FOV = abs(atan((X_obs(2,i)-X_pos(2))/(X_obs(1,i)-X_pos(1))) - atan(X_vel(2)/X_vel(1)));
       ...
       if SOD_metric > 0 
           ...
           for i = 1:4 % considering four obstacles;
               ...
               gradient_operator = [X_pos(1) - X_obs(1,i); X_pos(2) - X_obs(2,i)]/norm([X_pos(1) - X_obs(1,i); X_pos(2) - X_obs(2,i)]);
               
               % rotation angle 
               if norm([X_pos(1) - X_obs(1,i); X_pos(2) - X_obs(2,i)]) < rd && norm([X_pos(1) - X_obs(1,i); X_pos(2) - X_obs(2,i)]) >= rs
                   alpha_rotation = ((rd - norm([X_pos(1) - X_obs(1,i); X_pos(2) - X_obs(2,i)]))/(rd - rs))*pi/2;
               else 
                   alpha_rotation = 0;
               end
               
               % rotation matrix 
               Tr = [cos(alpha_rotation) -sin(alpha_rotation); sin(alpha_rotation) cos(alpha_rotation)];
               
               % Potentials Gradients
               delta_Vp = (norm(diag([kox,koy])*[X_pos(1) - X_obs(1,i); X_pos(2) - X_obs(2,i)]) - (rd + obs_radious))*gradient_operator;
               delta_Vr = kr*Tr*norm([X_pos(1) - X_obs(1,i); X_pos(2) - X_obs(2,i)])*gradient_operator;
 
               Upk(:,i) = delta_Vp;
               Urk(:,i) = delta_Vr;
               
           end
           Uo = -ko1*sum(Upk')' - sum(Urk')' - ko2*X_vel(1:2);
       else 
           Uo = zeros(2,1); 
       end
       ...
       if abs(X_pos(3)-5)<0.2
       for i = 1:NOA
           if i~=j
               prel = X_mat(1:2,i)-X_pos(1:2);
               vrel = X_mat(4:5,i)-X_vel(1:2);
               %~~~~~~~~~~~~~~~~
...
for k = 1:NOA
    Pos = Vehicles_Pos{k};
    plot3(Pos(1,1:i),Pos(2,1:i),Pos(3,1:i),':','linewidth',4)
    hold on
    grid on 
    plot3(Pos(1,i),Pos(2,i),Pos(3,i),'kx','linewidth',4)
    char = int2str(k);
...
[X,Y,Z] = cylinder([0.8*obs_radious,0.8*obs_radious],100);
surf(X+x_obs(1),Y+x_obs(2),10*Z, 'facealpha',1, 'edgealpha',1,'linewidth',2)
surf(X+x_obs2(1),Y+x_obs2(2),10*Z, 'facealpha',1, 'edgealpha',1,'linewidth',2)
surf(X+x_obs3(1),Y+x_obs3(2),10*Z, 'facealpha',1, 'edgealpha',1,'linewidth',2)
surf(X+x_obs4t(1,i),Y+x_obs4t(2,i),10*Z, 'facealpha',1, 'edgealpha',1,'linewidth',2)

plot(x_obs(1),x_obs(2),'^','markersize',6,'linewidth',3)
plot(x_obs2(1),x_obs2(2),'^','markersize',6,'linewidth',3)
plot(x_obs3(1),x_obs3(2),'^','markersize',6,'linewidth',3)
plot(x_obs4t(1,i),x_obs4t(2,i),'^','markersize',6,'linewidth',3)
